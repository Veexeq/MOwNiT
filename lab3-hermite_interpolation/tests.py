...
def custom_function_hermite_testing(a, b, n, func, deriv_func, equation, filename):
    ...
    x_unif = utils.generate_uniform_nodes(a, b, n)
    y_unif = func(x_unif)
    y_deriv_unif = deriv_func(x_unif)
        
    poly_unif_hermite = interpolators.hermite_formula(x_unif, y_unif, y_deriv_unif)
    y_interp_unif_hermite = poly_unif_hermite(x_dense)

    # 3. Chebyshev's nodes
    x_cheb = utils.generate_chebyshev_nodes(a, b, n)
    y_cheb = func(x_cheb)
    y_deriv_cheb = deriv_func(x_cheb)
        
    poly_cheb_hermite = interpolators.hermite_formula(x_cheb, y_cheb, y_deriv_cheb)
